...
<?php
        ...
        extract($_REQUEST);
         $sql = "SELECT `con_latitud1`,`con_longitud1`  FROM `tbl_contactos` WHERE `con_id`=$con_id AND `con_latitud1` IS NOT NULL AND `con_longitud1` IS NOT NULL ";

        $mapa = mysqli_query($conexion, $sql);
    if(mysqli_num_rows($mapa)>0){
            while($mapas    =   mysqli_fetch_array($mapa)){
              $latitud=$mapas['con_latitud1'];
              $longitud=$mapas['con_longitud1'];
            }
          }else{
              echo "<img src='http://4.bp.blogspot.com/-YseUoftHKQA/VLXRjVR7e5I/AAAAAAAAN18/6pB8E_Ta8_I/s1600/juego-google-chrome-offline.gif' height='350' width='1500''>";
              echo "<div align=center><b>NO TIENES UNA SEGUNDA DIRECCIÓN DISPONIBLE</b></div>";
